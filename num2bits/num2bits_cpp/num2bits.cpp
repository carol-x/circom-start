...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 9],&lvar[2]); // line circom 9
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_pow(&expaux[4],&circuitConstants[3],&lvar[2]); // line circom 14
Fr_mul(&expaux[2],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 0)],&expaux[4]); // line circom 14
Fr_add(&expaux[0],&lvar[1],&expaux[2]); // line circom 14
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 9]); // line circom 16
